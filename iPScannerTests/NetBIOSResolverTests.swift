...
final class NetBIOSResolverTests: XCTestCase {
    ...
    /// A real NBSTAT reply, captured off the wire from a Windows 11 host.
    ///
    /// This is the case the synthetic fixture below could not catch. Windows answers with
    /// QDCOUNT = 0 — it does not echo the question — while `makeResponse` builds QDCOUNT = 1 and
    /// echoes it, mirroring the parser's assumption instead of the protocol. So the suite passed
    /// while NetBIOS resolution failed against every real Windows host.
    private static let realWindowsReply = Data([
        ...
        0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x65, 0x03, 0x44, 0x45, 0x53,
        0x4b, 0x54, 0x4f, 0x50, 0x2d, 0x4e, 0x46, 0x4b, 0x44, 0x36, 0x38, 0x4e,
        0x00, 0x04, 0x00, 0x57, 0x4f, 0x52, 0x4b, 0x47, 0x52, 0x4f, 0x55, 0x50,
        0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x00, 0x84, 0x00, 0x44, 0x45, 0x53,
        0x4b, 0x54, 0x4f, 0x50, 0x2d, 0x4e, 0x46, 0x4b, 0x44, 0x36, 0x38, 0x4e,
        0x20, 0x04, 0x00, 0x00, 0x72, 0xee, 0x96, 0x53, 0xed, 0x00, 0x00, 0x00,
    ...
    func testParsesRealWindowsReplyWithoutEchoedQuestion() throws {
        let result = try XCTUnwrap(
            NetBIOSResolver.parseResponse(Self.realWindowsReply),
            "a real Windows NBSTAT reply must parse"
        )
        XCTAssertEqual(result.computerName, "DESKTOP-NFKD68N")
        XCTAssertEqual(result.workgroup, "WORKGROUP")
    }

    /// Guards the specific regression: the header's counts drive the layout, not a fixed shape.
    func testHonorsQuestionCountFromHeader() {
        XCTAssertEqual(Self.realWindowsReply[4], 0x00)
        XCTAssertEqual(Self.realWindowsReply[5], 0x00, "Windows sends QDCOUNT = 0")
        XCTAssertEqual(Self.realWindowsReply[7], 0x01, "ANCOUNT = 1")
    }

    /// A reply that claims no answers has nothing to read, whatever else it contains.
    func testRejectsReplyWithNoAnswers() {
        var bytes = Array(Self.realWindowsReply)
        bytes[6] = 0; bytes[7] = 0
        XCTAssertNil(NetBIOSResolver.parseResponse(Data(bytes)))
    }

    /// Builds a synthetic NBSTAT response containing the given list of
    /// (name, type, isGroup) entries plus a trailing 6-byte MAC.
    /// Note this shape — QDCOUNT = 1 with the question echoed — is what Samba does; Windows does
    /// not. Both must parse, which is why the captured reply above is tested alongside it.
